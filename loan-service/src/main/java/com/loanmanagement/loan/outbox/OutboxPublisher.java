...
public class OutboxPublisher {
    ...
    void registerMetrics() {
        meterRegistry.gauge(
                "loan_outbox_pending_events",
                this,
                publisher -> repository.countByStatus(OutboxEvent.Status.PENDING));
        meterRegistry.gauge(
                "loan_outbox_failed_events",
                this,
                publisher -> repository.countByStatus(OutboxEvent.Status.FAILED));
        meterRegistry.gauge(
                "loan_outbox_processing_events",
                this,
                publisher -> repository.countByStatus(OutboxEvent.Status.PROCESSING));
    }
    ...
    private void publish(OutboxEvent event) {
        try {
            ...
            if (retries >= MAX_RETRIES) {
                log.error(
                        "Outbox event permanently failed: id={}, type={}, retries={}",
                        event.getId(),
                        event.getEventType(),
                        retries,
                        ex);
                ...
                log.warn(
                        "Outbox publish failed: id={}, retry={}, nextRetryAt={}",
                        event.getId(),
                        retries,
    ...
    public void cleanupProcessedEvents() {
        ...
        if (deleted > 0) {
            log.info("Deleted {} processed outbox events older than 7 days", deleted);
        }
    }
